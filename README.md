...
`--navigation-demo` runs a confirmed A-to-B task as a real closed loop. Each timestamped `SensorFrame` contains the current pose and locally perceived obstacles; a replaceable navigation policy turns the latest frame into a short velocity action chunk, the robot adapter applies it, and an independent verifier checks arrival. The deterministic scenario begins on a clear path, introduces a crate after 600 ms, and proves that the policy replans from new observations instead of following a frozen startup trajectory. / `--navigation-demo` 将确认后的 A 到 B 任务作为真实闭环运行。每个带时间戳的 `SensorFrame` 包含当前位姿与局部感知障碍物；可替换 Navigation Policy 将最新帧转换成短时速度 Action Chunk，经 Robot Adapter 执行，并由独立 Verifier 检查到达。确定性场景开始时道路畅通，在 600ms 后出现箱体，用于证明策略会依据新观测重规划，而不是执行启动时冻结的轨迹。
