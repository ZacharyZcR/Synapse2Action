...
`--robot-transport loopback` moves command execution across the same serialized `base_velocity` request/acknowledgement boundary intended for a future ROS2 or chassis-SDK adapter. The loopback transport owns motion integration and collision rejection, so the complete intent → confirmation → VLA → action chunk → robot feedback path runs without hardware. Reports retain every command sequence, acceptance result, completion timestamp, pose, and base state. / `--robot-transport loopback` 会让动作执行真正跨过序列化的 `base_velocity` 请求/回执边界；未来 ROS2 或底盘 SDK adapter 复用同一接口。Loopback transport 负责运动积分和碰撞拒绝，因此无需设备也能跑通“意图 → 确认 → VLA → Action Chunk → 机器人反馈”全链路。报告保留每条命令的序号、接受结果、完成时间、位姿及底盘状态。
