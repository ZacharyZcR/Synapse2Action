...
The active execution order is defined by measured failures rather than phase numbering: first isolate the GR00T post-grasp lift failure with controlled counterfactuals and the pinned OpenWBT/OpenTrack matched baseline, then standardize the LeRobot policy boundary and qualify OpenPI or another mature policy under the same evidence contract, then demonstrate language causality across multiple TaskSpecs and bounded recovery, and only afterward begin physical-G1 preflight. See the [Ubuntu simulation runbook](docs/ubuntu-whole-body-simulation.md), [whole-body lift baseline](docs/whole-body-lift-baseline.md), and [industrialization gap](docs/industrialization-gap.md). / 当前执行顺序由实测失败决定，而不是机械地按阶段编号推进：先通过受控反事实及锁定版本的 OpenWBT/OpenTrack 匹配基线定位 GR00T 抓取后抬升失败，再统一 LeRobot Policy 边界并让 OpenPI 或另一成熟 Policy 通过同一证据契约，随后在多个 TaskSpec 上证明语言因果性与有界恢复，最后才进入 G1 真机预检。Ubuntu 执行步骤见[仿真运行手册](docs/ubuntu-whole-body-simulation.md)，实验门槛见[全身抬升基线](docs/whole-body-lift-baseline.md)和[工业化差距](docs/industrialization-gap.md)。
...
- [x] Unify SmolVLA and OpenPI proposals behind the bounded Action Chunk client contract; require an explicit observation encoder and embodiment mapper for OpenPI. / 将 SmolVLA 与 OpenPI 提案统一到受限 Action Chunk Client 契约；OpenPI 必须显式提供 Observation Encoder 与 Embodiment Mapper。
