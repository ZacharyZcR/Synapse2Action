...
function applyLanguage(){{
  const t=copy[lang], task=d.harness.task||{{}}, label=(task.display||{{}})[lang]||{{}}, provider=(d.harness.intelligence_stages||[]).find(s=>s.id==='llm_planner')?.provider||'Planner'; document.documentElement.lang=lang==='zh'?'zh-CN':'en';
  ...
  q('#verdict').classList.toggle('rejected',!d.harness.accepted); q('#verdictTitle').textContent=d.harness.accepted?t.successTitle:t.failedTitle; q('#verdictSummary').textContent=d.harness.accepted?(lang==='zh'?`EEG 选择${{label.target||task.arguments?.target}}；${{provider}} 生成 ${{task.skill}} 计划；SmolVLA、SDK2 与 MuJoCo 执行并验证“${{label.task||task.instruction}}”。`:`EEG selected ${{label.target||task.arguments?.target}}; ${{provider}} generated a ${{task.skill}} plan; SmolVLA, SDK2, and MuJoCo executed and verified “${{label.task||task.instruction}}.”`):t.failedSummary;
  ...
  const facts=[[t.standing,sim.fell_at_seconds===null],[t.grasped,sim.grasped],[t.lifted,(sim.maximum_object_height_m||0)-(sim.initial_object_position_xyz_m||[0,0,0])[2]>=(sim.verification?.minimum_lift_m||0)],[t.released,sim.released],[t.dropZone,sim.final_object_center_in_drop_zone],[t.authorized,sim.vla_authorized_frames>0]];
  ...
  q('#intelligenceStages').innerHTML=stages.length?stages.map(s=>{{const provenance=[s.provider,s.model].filter(Boolean).join(' · '), dynamicExplain=s.id==='intent'?(lang==='zh'?`从 EEG 解码“选择 ${{label.target||task.arguments?.target}}”，确认在计划审阅后才生效。`:`Decode “select ${{label.target||task.arguments?.target}}” from EEG; confirmation remains gated until plan review.`):s.id==='llm_planner'?(lang==='zh'?`${{provider}} 将任务转换为结构化 ${{task.skill}} 计划。`:`${{provider}} converts the task into a structured ${{task.skill}} plan.`):(t[s.id+'Explain']||'');return `<article class="stage-card"><header><h3>${{t[s.id]||s.id}}</h3><span class="provenance ${{s.mode}}">${{String(s.mode).toUpperCase()}}</span></header><p class="explain">${{dynamicExplain}}</p><strong class="${{s.status==='completed'?'ok':'warn'}}">${{s.status==='completed'?t.stageCompleted:t.stageFailed}}</strong>${{provenance?`<p class="mono">${{provenance}}</p>`:''}}<details><summary>${{t.viewEvidence}}</summary><dl><dt>${{t.input}}</dt><dd>${{describe(s.input)}}</dd><dt>${{t.output}}</dt><dd>${{describe(s.output)}}</dd>${{s.latency_ms===null||s.latency_ms===undefined?'':`<dt>${{t.latency}}</dt><dd>${{s.latency_ms}} ms</dd>`}}</dl></details></article>`}}).join(''):`<p class="warn">${{lang==='zh'?'旧报告未包含阶段证据，请重新运行仿真。':'Legacy report has no stage evidence. Run the simulation again.'}}</p>`;
