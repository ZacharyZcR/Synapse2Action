...
const copy={{zh:{{skip:'跳到主要内容',title:'Synapse2Action 仿真实验台',subtitle:'先看结论，再沿六个阶段理解机器人为什么成功或失败。',latestSnapshot:'最近一次验收快照',successTitle:'本次仿真运行成功',failedTitle:'本次仿真运行失败',successSummary:'合成 EEG 选择并确认红色方块；真实 DeepSeek 生成抓放计划；SmolVLA 授权技能；SDK2 控制仿真 G1 完成抓取、抬升、释放和放置。',failedSummary:'流程没有完整通过。下方阶段会标出停止位置，展开证据可查看该阶段的输入和输出。',snapshotLabel:'页面生成时间',realParts:'真实运行',realPartsValue:'DeepSeek、SmolVLA 推理、SDK2 控制链',simParts:'仿真替代',simPartsValue:'合成 EEG、MuJoCo 中的 G1 与环境',fixedParts:'确定性部分',fixedPartsValue:'C++ 抓放 Behavior 与物理验收',limitation:'关键限制',limitationValue:'SmolVLA 当前只授权技能，没有生成抓取位姿或关节动作。',advancedMetrics:'查看专业运行指标',viewEvidence:'查看输入与输出证据',stageCompleted:'阶段完成',stageFailed:'阶段失败',intentExplain:'从合成 EEG 中解码“选择红色方块”，并等待第二次确认后才允许继续。',llm_plannerExplain:'真实 DeepSeek 把已确认目标转换成结构化抓放计划。',vlaExplain:'SmolVLA读取三路相机和关节状态，返回 Action Chunk；当前仅作为技能授权窗口。',skill_executorExplain:'确定性 C++ Behavior 校验计划，并生成受约束的抓取、搬运和释放目标。',motion_controlExplain:'Unitree RL Controller 保持平衡，SDK2 将低层控制帧送入 MuJoCo。',physical_verificationExplain:'读取机器人与物体状态，独立检查站立、抓取、抬升、释放和落盘。',intelligenceStages:'这次运行经历了什么',evidenceRule:'按实际执行顺序阅读',status:'状态',input:'收到',output:'产出',latency:'耗时',noFrames:'本次运行未产生 MuJoCo 相机帧，请查看失败阶段。',intent:'01 · 意图与确认',llm_planner:'02 · LLM 规划',vla:'03 · VLA 感知与输出',skill_executor:'04 · 技能校验与执行',motion_control:'05 · 运动控制',physical_verification:'06 · 物理验证',harnessState:'Harness 状态',maxRtt:'VLA 最大 RTT',coverage:'Chunk 覆盖',stale:'过期回退',camera:'MuJoCo 相机 01',previous:'上一帧',next:'下一帧',play:'播放',pause:'暂停',trace:'Harness 事件链',ownership:'控制权归属',overlay:'生成式覆盖 = 0',authorization:'类型化技能授权',targets:'经过验证的操作目标',lowLevel:'平衡与低层控制',intents:'EEG 解码意图',budget:'VLA 实时预算',outcome:'物理结果',verified:'独立验证',quality:'留出集模型质量',overallMse:'整体 MSE',armMse:'腰部 / 手臂 MSE',frames:'帧数',finite:'有限值',disclaimer:'这是最近一次运行的离线报告快照，不是实时监控，也不代表真实 EEG 或 G1 硬件性能。',chunks:'个 CHUNK',budgetWord:'预算',pass:'通过',fail:'失败',ready:'就绪',grasp:'抓取',lift:'抬升',release:'释放',final:'最终',standing:'保持站立',grasped:'已抓取',lifted:'已抬升',released:'已释放',dropZone:'进入放置区',authorized:'VLA 已授权'}},en:{{skip:'Skip to main content',title:'Synapse2Action Simulation Console',subtitle:'Read the verdict first, then follow six stages to understand why the robot succeeded or failed.',latestSnapshot:'Latest acceptance snapshot',successTitle:'This simulation run succeeded',failedTitle:'This simulation run failed',successSummary:'Synthetic EEG selected and confirmed the red cube; live DeepSeek produced a pick-and-place plan; SmolVLA authorized the skill; SDK2 controlled the simulated G1 through grasp, lift, release, and placement.',failedSummary:'The pipeline did not complete. The stages below show where it stopped; expand evidence to inspect that stage’s input and output.',snapshotLabel:'Page generated',realParts:'Live components',realPartsValue:'DeepSeek, SmolVLA inference, and SDK2 control path',simParts:'Simulated substitutes',simPartsValue:'Synthetic EEG and the G1/world in MuJoCo',fixedParts:'Deterministic parts',fixedPartsValue:'C++ pick-place behavior and physical acceptance',limitation:'Key limitation',limitationValue:'SmolVLA currently authorizes a skill; it does not generate grasp poses or joint commands.',advancedMetrics:'Show advanced runtime metrics',viewEvidence:'View input and output evidence',stageCompleted:'Stage completed',stageFailed:'Stage failed',intentExplain:'Decode “select red cube” from synthetic EEG and require a second confirmation before proceeding.',llm_plannerExplain:'Live DeepSeek converts the confirmed target into a structured pick-and-place plan.',vlaExplain:'SmolVLA reads three cameras and joint state and returns Action Chunks; these currently act as a skill authorization window.',skill_executorExplain:'Deterministic C++ behavior validates the plan and produces constrained grasp, transport, and release targets.',motion_controlExplain:'The Unitree RL controller maintains balance while SDK2 sends low-level control frames into MuJoCo.',physical_verificationExplain:'Robot and object state independently verify standing, grasp, lift, release, and drop-zone placement.',intelligenceStages:'What happened in this run',evidenceRule:'Read in execution order',status:'Status',input:'Received',output:'Produced',latency:'Latency',noFrames:'This run produced no MuJoCo camera frames. Inspect the failed stage.',intent:'01 · Intent and confirmation',llm_planner:'02 · LLM planning',vla:'03 · VLA perception and output',skill_executor:'04 · Skill validation and execution',motion_control:'05 · Motion control',physical_verification:'06 · Physical verification',harnessState:'Harness state',maxRtt:'VLA max RTT',coverage:'Chunk coverage',stale:'Stale fallback',camera:'MuJoCo Camera 01',previous:'Previous',next:'Next',play:'Play',pause:'Pause',trace:'Harness event trace',ownership:'Control ownership',overlay:'GENERATIVE OVERLAY = 0',authorization:'typed-skill authorization',targets:'validated manipulation targets',lowLevel:'balance and low-level control',intents:'EEG decoded intents',budget:'VLA realtime budget',outcome:'Physical outcome',verified:'INDEPENDENTLY VERIFIED',quality:'Held-out model quality',overallMse:'Overall MSE',armMse:'Waist / arm MSE',frames:'Frames',finite:'Finite',disclaimer:'This is an offline snapshot of the latest run, not live monitoring or evidence of physical EEG/G1 performance.',chunks:'CHUNKS',budgetWord:'budget',pass:'PASS',fail:'FAIL',ready:'READY',grasp:'GRASP',lift:'LIFT',release:'RELEASE',final:'FINAL',standing:'standing',grasped:'grasped',lifted:'lifted',released:'released',dropZone:'in drop zone',authorized:'VLA authorized'}}}};
