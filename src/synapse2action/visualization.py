...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .stage-grid {{ display:grid; grid-template-columns:repeat(6,minmax(170px,1fr)); gap:8px; overflow-x:auto; padding-bottom:6px; }}
    .stage-card {{ min-height:178px; padding:12px; border:1px solid var(--line); border-radius:10px; background:#0b1324; }}
    ...
    @media (max-width:600px) {{ main {{ width:min(100% - 20px,1500px); padding-top:14px; }} header {{ display:block; }} .header-actions {{ margin-top:14px; }} .metrics {{ grid-template-columns:1fr 1fr; }} .thumbs {{ grid-template-columns:repeat(3,1fr); }} .topology {{ grid-template-columns:1fr; }} .node:not(:last-child)::after {{ display:none; }} }}
    @media (prefers-reduced-motion:reduce) {{ *,*::before,*::after {{ scroll-behavior:auto!important; transition:none!important; animation:none!important; }} }}
  </style>
</head>
<body>
...
<main id="main">
  ...
  <div class="grid">
    <div class="stack">
      ...
      <section class="card"><div class="card-head"><h2 data-i18n="ownership">控制权归属</h2><span class="mono ok" data-i18n="overlay">生成式覆盖 = 0</span></div><div class="topology"><div class="node"><b>SmolVLA · 3Hz</b><small data-i18n="authorization">类型化技能授权</small></div><div class="node"><b>C++ Behavior</b><small data-i18n="targets">经过验证的操作目标</small></div><div class="node"><b>SDK2 + RL</b><small data-i18n="lowLevel">平衡与低层控制</small></div></div></section>
    </div>
    <aside class="stack">
...
const copy={{zh:{{skip:'跳到主要内容',title:'Synapse2Action 仿真实验台',subtitle:'EEG 意图、LLM 规划、VLA 输出、技能执行、Unitree 控制与 MuJoCo 物理结果的分阶段证据。',intelligenceStages:'智能链路阶段',evidenceRule:'每阶段必须显示来源与证据',status:'状态',input:'输入',output:'输出',latency:'耗时',noFrames:'本次运行未产生 MuJoCo 相机帧，请查看失败阶段。',intent:'01 · 意图与确认',llm_planner:'02 · LLM 规划',vla:'03 · VLA 感知与输出',skill_executor:'04 · 技能校验与执行',motion_control:'05 · 运动控制',physical_verification:'06 · 物理验证',harnessState:'Harness 状态',maxRtt:'VLA 最大 RTT',coverage:'Chunk 覆盖',stale:'过期回退',camera:'MuJoCo 相机 01',previous:'上一帧',next:'下一帧',play:'播放',pause:'暂停',trace:'Harness 事件链',ownership:'控制权归属',overlay:'生成式覆盖 = 0',authorization:'类型化技能授权',targets:'经过验证的操作目标',lowLevel:'平衡与低层控制',intents:'EEG 解码意图',budget:'VLA 实时预算',outcome:'物理结果',verified:'独立验证',quality:'留出集模型质量',overallMse:'整体 MSE',armMse:'腰部 / 手臂 MSE',frames:'帧数',finite:'有限值',disclaimer:'本页面由本地验收报告生成，不访问网络，不代表真实 EEG 或 G1 硬件性能。',chunks:'个 CHUNK',budgetWord:'预算',pass:'通过',fail:'失败',ready:'就绪',grasp:'抓取',lift:'抬升',release:'释放',final:'最终',standing:'保持站立',grasped:'已抓取',lifted:'已抬升',released:'已释放',dropZone:'进入放置区',authorized:'VLA 已授权'}},en:{{skip:'Skip to main content',title:'Synapse2Action Simulation Console',subtitle:'Stage-by-stage evidence for EEG intent, LLM planning, VLA output, skill execution, Unitree control, and MuJoCo outcomes.',intelligenceStages:'Intelligence pipeline stages',evidenceRule:'Every stage exposes provenance and evidence',status:'Status',input:'Input',output:'Output',latency:'Latency',noFrames:'This run produced no MuJoCo camera frames. Inspect the failed stage.',intent:'01 · Intent and confirmation',llm_planner:'02 · LLM planning',vla:'03 · VLA perception and output',skill_executor:'04 · Skill validation and execution',motion_control:'05 · Motion control',physical_verification:'06 · Physical verification',harnessState:'Harness state',maxRtt:'VLA max RTT',coverage:'Chunk coverage',stale:'Stale fallback',camera:'MuJoCo Camera 01',previous:'Previous',next:'Next',play:'Play',pause:'Pause',trace:'Harness event trace',ownership:'Control ownership',overlay:'GENERATIVE OVERLAY = 0',authorization:'typed-skill authorization',targets:'validated manipulation targets',lowLevel:'balance and low-level control',intents:'EEG decoded intents',budget:'VLA realtime budget',outcome:'Physical outcome',verified:'INDEPENDENTLY VERIFIED',quality:'Held-out model quality',overallMse:'Overall MSE',armMse:'Waist / arm MSE',frames:'Frames',finite:'Finite',disclaimer:'Generated from local acceptance reports. No network access. Not evidence of physical EEG or G1 performance.',chunks:'CHUNKS',budgetWord:'budget',pass:'PASS',fail:'FAIL',ready:'READY',grasp:'GRASP',lift:'LIFT',release:'RELEASE',final:'FINAL',standing:'standing',grasped:'grasped',lifted:'lifted',released:'released',dropZone:'in drop zone',authorized:'VLA authorized'}}}};
...
function applyLanguage(){{const t=copy[lang];document.documentElement.lang=lang==='zh'?'zh-CN':'en';document.querySelectorAll('[data-i18n]').forEach(x=>x.textContent=t[x.dataset.i18n]);q('#language').textContent=lang==='zh'?'EN':'中文';q('#language').setAttribute('aria-label',lang==='zh'?'Switch to English':'切换到中文');q('#chunkCount').textContent=runtime.chunks_received+' '+t.chunks;q('#coverageText').textContent=t.budgetWord+' '+Math.round(runtime.minimum_chunk_coverage_ms)+' ms';q('#finite').textContent=d.training.finite?t.pass:t.fail;const facts=[[t.standing,sim.fell_at_seconds===null],[t.grasped,sim.grasped],[t.lifted,(sim.maximum_object_height_m||0)-(sim.initial_object_position_xyz_m||[0,0,0])[2]>=.1],[t.released,sim.released],[t.dropZone,sim.final_object_center_in_drop_zone],[t.authorized,sim.vla_authorized_frames>0]];q('#facts').innerHTML=facts.map(([name,ok])=>`<div class="fact"><small>${{name}}</small><b class="${{ok?'ok':'warn'}}">${{ok?t.pass:t.fail}}</b></div>`).join('');const stages=d.harness.intelligence_stages||[];q('#intelligenceStages').innerHTML=stages.length?stages.map(s=>`<article class="stage-card"><header><h3>${{t[s.id]||s.id}}</h3><span class="provenance ${{s.mode}}">${{String(s.mode).toUpperCase()}}</span></header><dl><dt>${{t.status}}</dt><dd class="${{s.status==='completed'?'ok':'warn'}}">${{String(s.status).toUpperCase()}}</dd><dt>${{t.input}}</dt><dd>${{describe(s.input)}}</dd><dt>${{t.output}}</dt><dd>${{describe(s.output)}}</dd>${{s.latency_ms===null||s.latency_ms===undefined?'':`<dt>${{t.latency}}</dt><dd>${{s.latency_ms}} ms</dd>`}}</dl></article>`).join(''):`<p class="warn">${{lang==='zh'?'旧报告未包含阶段证据，请重新运行仿真。':'Legacy report has no stage evidence. Run the simulation again.'}}</p>`;q('#thumbnails').innerHTML=frames.map((f,i)=>`<button class="thumb" type="button" data-index="${{i}}"><img src="${{f.data}}" alt=""><span>${{stageName(f.stage)}}</span></button>`).join('');document.querySelectorAll('.thumb').forEach(x=>x.onclick=()=>{{stop();renderFrame(Number(x.dataset.index))}});renderFrame(frameIndex);stop();}}
