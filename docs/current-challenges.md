...
12. **Joint-space chunk admission is bounded; Cartesian workspace admission is not.** Complete chunks are now rejected before controller loading for joint-position, velocity, acceleration, or duration violations. A 29-joint vector does not by itself prove end-effector clearance, self-collision, or scene collision; those checks remain gated on a trusted embodiment-specific kinematics and collision model.
13. **Project policies now exist, but confidential intake is not externally verified.** Contribution, governance, security, privacy, and responsible-use rules are versioned and machine-checked. GitHub's private-vulnerability-reporting API returned `404` on 2026-08-31, so the repository owner must verify or enable that setting before advertising it as an available reporting channel.
...
12. **关节空间 Chunk 准入已受限，但笛卡尔工作空间准入尚未完成。** 整段 Chunk 现在会在装入 Controller 前检查关节位置、速度、加速度与持续时间，任一越界即整体拒绝。仅凭 29 维关节向量无法证明末端间隙、自碰撞或场景碰撞；这些检查仍需可信的本体专用运动学与碰撞模型。
13. **项目政策已经建立，但私密安全入口尚未得到外部验证。** 贡献、治理、安全、隐私与负责任使用规则已经版本化并通过测试检查。GitHub Private Vulnerability Reporting 面向公开仓库；Private 状态下的 `404` 不能判定启用失败。仓库所有者应在公开后启用并验证该入口，再对外宣称可用。
...
5. Feed every candidate's matched run and language evidence through the Policy Admission Verifier; do not promote a policy from upstream claims or isolated success.
6. Measure recovery success and failure-detection recall when candidate-policy runtimes are available; preserve the accepted hardware-free state-machine gate.
...
5. 将每个候选 Policy 的匹配运行与语言证据送入 Policy Admission Verifier；不得凭上游声明或孤立成功晋级。
6. 在候选 Policy Runtime 可用后测量恢复成功率与失败检测召回率；保留已验收的无硬件状态机门槛。
7. 只有在只读预检、实体急停、受控工作区和人工接管就绪后，才开始 G1 真机工作。
8. 在仓库设置中检查或启用 GitHub Private Vulnerability Reporting；敏感报告不得转入公开 Issue。
