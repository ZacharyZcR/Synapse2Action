...
The structured Planner action now crosses the subprocess boundary and is recorded as the VLA task binding. A real trained SmolVLA run received three chunks with a 10.91-second first-chunk latency inside 16.67 seconds of coverage and zero stale fallbacks. Under the same observation and seed, changing the language plan produced a 0.02262-rad RMS action difference across a 50-frame, 29-dimensional chunk. The LLM-to-VLA boundary therefore passes. This validates transport and language conditioning, not that an LLM is necessary for the single fixed task; the current accepted binding run used `MockPlanner`, so a live-LLM repetition remains required before making a production-LLM claim.

现在结构化 Planner Action 已跨越子进程边界，并作为 VLA Task Binding 写入报告。真实训练后的 SmolVLA 收到 3 个 Chunk，首 Chunk 延迟 10.91 秒，低于 16.67 秒覆盖窗口，且无 Stale Fallback；在观测和随机种子完全相同的条件下，更换语言 Plan 后，50 帧、29 维 Action Chunk 的 RMS 差异为 0.02262rad。因此 LLM→VLA 边界已经通过。这证明了数据传递和语言条件作用，但尚未证明固定单任务必须使用 LLM；当前通过绑定验收的运行使用 `MockPlanner`，在声称真实生产 LLM 前仍需用 Live LLM 重复一次。
