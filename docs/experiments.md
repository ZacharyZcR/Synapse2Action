...
The online acceptance keeps three boundaries separate: SmolVLA provides a 3 Hz typed-skill authorization window, the validated C++ behavior in the Unitree controller generates manipulation targets, and the official SDK2/RL stack owns low-level control. The behavior clock starts only after the first VLA authorization arrives. Camera acquisition time is included in end-to-end chunk latency, and later chunks are inferred concurrently. Release requires prior lift, full object support inside the tray footprint, and a lowered object. Functional and realtime gates must both pass; the bridge separately reports authorization frames and generative joint-overlay frames.

在线验收严格分离三层边界：SmolVLA 以 3Hz 提供类型化技能授权窗口，Unitree Controller 内已验证的 C++ Behavior 生成操作目标，官方 SDK2/RL 栈拥有低层控制权。首个 VLA 授权到达后行为时钟才启动；相机采集耗时计入 chunk 端到端时延，后续 chunk 并发推理。释放必须同时满足“已抬升、物体底面完整进入托盘、已下降”。功能门与实时门必须同时通过，Bridge 分别报告授权帧和生成式关节覆盖帧。
