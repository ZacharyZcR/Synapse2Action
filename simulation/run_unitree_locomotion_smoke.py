...
def run(project: Path, duration: float, minimum_height: float, minimum_distance: float) -> dict[str, object]:
    try:
        import mujoco
        import numpy as np
        import torch
...
def main() -> None:
    ...
    try:
        report = run(args.project.resolve(), args.duration, args.minimum_height, args.minimum_distance)
