...
class PublicSSVEPAssetsTests(unittest.TestCase):
    ...
    def test_benchmark_is_subject_independent_and_replays_harness(self) -> None:
        ...
        self.assertIn("filter_bank_cca_scores", source)
        self.assertIn('"continuous_stream"', source)
        self.assertIn('"idle_false_activation_rate": None', source)
        self.assertIn('stream["false_events_per_minute"] <= 1.0', source)
        self.assertIn('"diagnostic_only_selected_examples"', source)
